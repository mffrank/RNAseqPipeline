...
if [ "$PARAMS" == "" ]; then
	...
	echo "";
	echo "rna-seq-pipeline PAIRED[0|1] STAGE[0:4]  RUN_NAME OUT_DIR Path/to/parameter_file";
	echo "";
...
if [ $PAIRED -eq 1 ]; then
	...
	if [ $STAGE -le 4 ]; then
		...
		bsub -n $cl_threads -J job7 -w "done(job6)" -o $OUT_DIR/cufflinks/${RUN_NAME}_cufflinks_assembly_euler_log.txt -e $OUT_DIR/cufflinks/${RUN_NAME}_cufflinks_assembly_log.txt cufflinks -p $cl_threads -G $annotation --library-type $librarytype $OUT_DIR/samtools/${RUN_NAME}Aligned.out_sorted.bam --output-dir $OUT_DIR/cufflinks
